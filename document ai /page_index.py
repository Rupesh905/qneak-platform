...
def calculate_page_offset(pairs):
    ...
    for pair in pairs:
        try:
            ...
            difference = int(physical_index) - int(page_number)
...
def add_page_number_to_toc(part, structure, model=None):
    """
    Add page numbers to TOC items with defensive type checking.
    Fixes: 'str' object does not support item deletion error
    """
    fill_prompt_seq = """
    ...
    # FIX 1: Handle case where json_result is not a list
    if not isinstance(json_result, list):
        print(f"Warning: Expected list from LLM, got {type(json_result)}. Converting to list.")
        if isinstance(json_result, dict):
            json_result = [json_result]
        else:
            # If it's a string or other type, return empty list or wrap it
            print(f"Warning: Unexpected type {type(json_result)} from LLM, returning empty list")
            return []
    
    # FIX 2: Handle case where items in list are strings or None (the main bug fix)
    cleaned_result = []
    for item in json_result:
        if isinstance(item, dict):
            # Only delete 'start' if it's a dictionary and has the key
            if 'start' in item:
                del item['start']
            cleaned_result.append(item)
        else:
            print(f"Warning: Skipping non-dictionary item in result: {item} (type: {type(item)})")
            # Skip non-dictionary items - don't try to process them
...
def process_none_page_numbers(toc_items, page_list, start_index=1, model=None):
    ...
    Fixed to handle type checking properly.
    """
    for i, item in enumerate(toc_items):
        # FIX: Check if item is a dictionary before processing
        if not isinstance(item, dict):
            print(f"Warning: Skipping non-dict item in process_none_page_numbers: {item}")
            continue
        ...
        if "physical_index" not in item:
            ...
            for j in range(i - 1, -1, -1):
                if j < len(toc_items) and isinstance(toc_items[j], dict) and toc_items[j].get('physical_index') is not None:
            ...
            # FIX: Create safe copy and check type before deleting
            item_copy = copy.deepcopy(item)
            if isinstance(item_copy, dict) and 'page' in item_copy:
                del item_copy['page']
            
            result = add_page_number_to_toc(page_contents, item_copy, model)
            
            # FIX: Ensure result is valid before accessing
            if not isinstance(result, list) or len(result) == 0:
                continue
                
            if isinstance(result[0], dict) and isinstance(result[0].get('physical_index'), str) and result[0]['physical_index'].startswith('<physical_index'):
